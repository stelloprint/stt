...
const capabilitiesPath = join(rootDir, "apps/web/src-tauri/capabilities/default.json");
const cargoTomlPath = join(rootDir, "apps/web/src-tauri/Cargo.toml");

const REQUIRED_CSP = "default-src 'self'; connect-src 'none'; img-src 'self'; style-src 'self' 'unsafe-inline'";
const FORBIDDEN_PLUGINS = ["tauri-plugin-shell", "tauri-plugin-http", "tauri-plugin-process", "tauri-plugin-fs"];
